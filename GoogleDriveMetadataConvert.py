...
def rename_googledocs(): #renames google docs with appropriate new filname for download, always leaves a note to state which format it is converting it to.
    filelist['file_name'] = np.where(filelist.mimeType == 'application/vnd.google-apps.document', filelist['file_name'] + '.odt', filelist['file_name'])
    filelist['archivist_note'] = np.where(filelist.mimeType == 'application/vnd.google-apps.document', 'This file was originally a cloud google format and has been converted to an OpenDocument Text file', filelist['archivist_note'])
    filelist['file_name'] = np.where(filelist.mimeType == 'application/vnd.google-apps.spreadsheet', filelist['file_name'] + '.ods', filelist['file_name'])
    filelist['archivist_note'] = np.where(filelist.mimeType == 'application/vnd.google-apps.spreadsheet', 'This file was originally a cloud google format and has been converted to an OpenDocument Spreadsheet file', filelist['archivist_note'])
    filelist['file_name'] = np.where(filelist.mimeType == 'application/vnd.google-apps.presentation', filelist['file_name'] + '.odp', filelist['file_name'])
    filelist['archivist_note'] = np.where(filelist.mimeType == 'application/vnd.google-apps.presentation', 'This file was originally a cloud google format and has been converted to an OpenDocument Presentation file', filelist['archivist_note'])
    ...
    filelist['archivist_note'] = np.where(filelist.mimeType == 'application/vnd.google-apps.drawing', 'This file was originally a cloud google format and has been converted to a PNG file', filelist['archivist_note'])
    filelist['file_name'] = np.where(filelist.mimeType == 'application/vnd.google-apps.jam',filelist['file_name'] + '.pdf', filelist['file_name'])
    filelist['archivist_note'] = np.where(filelist.mimeType == 'application/vnd.google-apps.jam', 'This file was originally a cloud google format and has been converted to a PDF file', filelist['archivist_note'])


rename_googledocs()

def rename_problem_files(): #renames caracters not allowed in file systems with _, always leaves a note to say when filename has changed.
    filelist['file_name'] = filelist['file_name'].str.replace("/","_")
    filelist['file_name'] = filelist['file_name'].str.replace("\\", "_")
    filelist['file_name'] = filelist['file_name'].str.replace(":", "_")
    filelist['file_name'] = filelist['file_name'].str.replace("*", "_")
    filelist['file_name'] = filelist['file_name'].str.replace("?", "_")
    ...
    filelist['file_name'] = filelist['file_name'].str.replace("<", "_")
    filelist['file_name'] = filelist['file_name'].str.replace("|", "_")
    filelist['file_name_note'] = np.where(filelist.file_name != filelist.original_file_name,'This filename has been adjusted from the original',filelist['file_name_note'])
...
def rename_duplicates(): #renames duplicate files with numerical number
    ...
    filelist['file_name'] = filelist['file_name'].str.replace("\(0\)","")
rename_duplicates()
...
def rename_folders(): #takes the identifier converts google id to file or folder name, then adds slashes, removes speech marks, creates folder column with folder or fie entries depending on mime type.
    ...
    filelist['identifier'] = filelist['identifier'].str.lstrip("('").str.replace("'\)",'/').str.replace("', '","/").str.replace("',\)",'/').str.lstrip(')')
...
content = content[
        ['identifier', 'file_name','original_file_name', 'folder', 'date_created', 'date_last_modified','checksum_md5', 'closure_type',
         'closure_period', 'closure_start_date', 'foi_exemption_code', 'foi_exemption_asserted', 'title_public',
         'title_alternate', 'google_id', 'google_parent_id', 'rights_copyright', 'legal_status',
         'held_by', 'mimeType','size', 'archivist_note','file_name_note']]
