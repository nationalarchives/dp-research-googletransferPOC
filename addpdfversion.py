...
filelist = pd.read_csv('H:/GoogleTestMetadataTest.csv', encoding='utf8')
...
def add_pdf_version(): #creates a sub dataframe to add a pdf version entry for all Google formats and renames with appropriate extension. This is then merged with the main dataframe
    ...
    pdfcopy['identifier'] = np.where(pdfcopy.archivist_note == 'This file was originally a Google Doc format and has been converted to an Microsoft Office Word file', pdfcopy['identifier'].str.replace('.xslx', '.pdf', regex=True), pdfcopy['identifier'])
    ...
    pdfcopy['identifier'] = np.where(pdfcopy.archivist_note == 'This file was originally a Google Doc format and has been converted to an Microsoft Office Word file', pdfcopy['identifier'].str.replace('.pptx', '.pdf', regex=True), pdfcopy['identifier'])
...
content.to_csv('H:/GoogleTestMetadataPDF.csv', index=False)
